...
async function loadNews() {
  try {
    const res = await axios.get('http://localhost:8080/api/tin-tuc?size=6')
    const newsList = res.data.content || []
    news.value = newsList.map(item => {
      ...
      const plainText = item.noiDung ? item.noiDung.replace(/<[^>]*>/g, '') : ''
      const summaryText = plainText.length > 150 ? plainText.substring(0, 150) + '...' : plainText
      
      return {
        id: item.id,
        image: item.anhDaiDien || 'https://images.unsplash.com/photo-1516549655169-df83a0774514?auto=format&fit=crop&w=600&q=80',
        title: item.tieuDe,
        summary: summaryText || 'Không có mô tả chi tiết.',
        date: formattedDate,
...
async function loadDoctors() {
  try {
    ...
    doctors.value = staffList.map((s, index) => {
      return {
        name: s.hoTen,
        specialty: 'Bác sĩ chuyên khoa',
        experience: 'Bác sĩ điều trị',
        image: defaultImages[index % defaultImages.length],
        desc: 'Bác sĩ điều trị tận tâm, giàu kinh nghiệm khám và chăm sóc sức khỏe.'
      }
    })
  } catch (err) {
    console.error('Lỗi khi tải bác sĩ trang chủ:', err)
  }
}
