...
function handleSendOTP() {
  ...
  alert(`Mã OTP đã được gửi về: ${info.value}. Mã mặc định là 123456.`)
  step.value = 2
}
...
function handleResetPassword() {
  ...
  // Update password in local storage if user exists
  let registeredUsers = JSON.parse(localStorage.getItem('registeredUsers') || '[]')
  let index = registeredUsers.findIndex(u => u.phone === info.value || u.email === info.value)
  if (index !== -1) {
    registeredUsers[index].password = newPassword.value
