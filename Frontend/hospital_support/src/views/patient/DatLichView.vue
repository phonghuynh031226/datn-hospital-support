...
const form = ref({
  ...
  doctor: '',     // Doctor ID (Integer) or empty
  date: '',
  timeSlot: '',
  symptoms: '',
...
const activeBookingResult = ref(null)
...
onMounted(async () => {
  ...
  try {
    ...
    console.error('Lỗi khi tải danh mục khoa:', err)
  }
...
async function onDepartmentChange() {
  ...
  try {
    const res = await axios.get(`http://localhost:8080/api/bac-si?departmentId=${form.value.department}`)
...
function getDepartmentName(id) {
  const found = departmentList.value.find(d => d.id === id)
  return found ? found.tenKhoa : 'Chưa chọn'
}

function getDoctorName(id) {
  if (!id) return 'Bác sĩ bất kỳ'
  const found = doctorList.value.find(d => d.id === id)
  return found ? found.hoTen : 'Bác sĩ bất kỳ'
}

function mapStatusText(status) {
  if (status === 'CHO_XAC_NHAN') return 'Chờ xác nhận (Thanh toán tại quầy)'
  if (status === 'DA_XAC_NHAN') return 'Đã xác nhận (Đã thanh toán)'
...
watch(() => form.value.symptoms, (newVal) => {
  ...
  if (val.includes('ngực') || val.includes('tim') || val.includes('huyết áp') || val.includes('mạch') || val.includes('hồi hộp') || val.includes('xơ vữa')) {
    ...
    suggested = 'Nội tổng quát'
    confidence = 80
  }
  
  if (suggested) {
    aiSuggestedDept.value = suggested
    aiConfidence.value = confidence
    const found = departmentList.value.find(d => d.tenKhoa.toLowerCase().includes(suggested.toLowerCase()))
    if (found) {
      form.value.department = found.id
      onDepartmentChange()
    }
  } else {
    aiSuggestedDept.value = ''
...
async function confirmBooking() {
  ...
  let docId = form.value.doctor
  if (!docId) {
    if (doctorList.value.length > 0) {
      docId = doctorList.value[0].id
    } else {
      alert('Khoa này hiện chưa có bác sĩ trực. Vui lòng chọn khoa khác hoặc liên hệ hotline!')
      return
    }
  }
  ...
  const requestBody = {
    ...
    doctorId: docId,
    ngayHen: form.value.date,
    khungGio: form.value.timeSlot,
    trieuChung: form.value.symptoms || 'Khám tổng quát định kỳ'
  }
  
  try {
    const res = await axios.post('http://localhost:8080/api/lich-hen', requestBody)
    ...
    alert(err.response?.data?.message || 'Đã xảy ra lỗi khi tạo lịch khám!')
  }
}
...
async function completeOnlinePayment() {
  ...
  try {
    const res = await axios.put(`http://localhost:8080/api/lich-hen/${tempBookingData.value.id}/pay`)
    activeBookingResult.value = res.data
    ...
    alert(err.response?.data?.message || 'Có lỗi xảy ra trong quá trình thanh toán!')
  }
}
...
async function cancelBooking(id) {
  ...
  try {
    ...
    alert(err.response?.data?.message || 'Lỗi khi hủy lịch khám!')
  }
}
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                >
                  ...
                  <option v-for="d in departmentList" :key="d.id" :value="d.id">{{ d.tenKhoa }}</option>
                </select>
              </div>
              ...
              <div>
                ...
                >
                  ...
                  <option v-for="doc in doctorList" :key="doc.id" :value="doc.id">{{ doc.hoTen }}</option>
                </select>
              </div>
            </div>
        ...
        <div class="lg:col-span-5 space-y-6">
          ...
          <div v-if="activeBookingResult" class="bg-gradient-to-br from-primary-600 to-primary-800 text-white p-6 md:p-8 rounded-3xl shadow-xl animate-fade-in space-y-6 relative overflow-hidden border border-primary-700">
            ...
            <div class="text-center border-b border-white/20 pb-4">
              ...
              <h3 class="text-3xl font-extrabold mt-3 tracking-widest">LH-{{ activeBookingResult.id }}</h3>
            ...
            <div class="space-y-4 text-base">
              <div class="flex justify-between border-b border-white/10 pb-2">
                ...
                <span class="font-bold text-lg">{{ activeBookingResult.patientName }}</span>
              </div>
              <div class="flex justify-between border-b border-white/10 pb-2">
                <span class="text-blue-100">Bác sĩ:</span>
                <span class="font-bold">{{ activeBookingResult.doctorName }}</span>
              </div>
              <div class="flex justify-between border-b border-white/10 pb-2">
                <span class="text-blue-100">Khung giờ:</span>
                <span class="font-bold text-yellow-300">{{ activeBookingResult.khungGio }}</span>
              </div>
              <div class="flex justify-between border-b border-white/10 pb-2">
                <span class="text-blue-100">Ngày khám:</span>
                <span class="font-bold">{{ activeBookingResult.ngayHen }}</span>
              </div>
              <div class="flex justify-between border-b border-white/10 pb-2">
                ...
                <span class="font-bold">
                  ...
                  <span class="text-xs px-2 py-0.5 rounded-full ml-1 font-bold" 
                        :class="activeBookingResult.trangThai === 'DA_XAC_NHAN' ? 'bg-emerald-500/30 text-emerald-300' : 'bg-amber-500/30 text-amber-300'">
                    ({{ mapStatusText(activeBookingResult.trangThai) }})
                  </span>
                </span>
              </div>
              ...
              <div class="p-4 bg-white text-primary-900 rounded-2xl flex items-center justify-between shadow-sm">
                <div>
                  <span class="text-xs text-gray-400 font-bold uppercase">Số thứ tự khám (ước tính)</span>
                  <h4 class="text-4xl font-extrabold text-primary-800 mt-1">{{ activeBookingResult.id % 30 + 1 }}</h4>
                </div>
                <div class="text-right text-xs text-gray-400">
                  <p>Hãy tới sảnh phòng khám</p>
                  <p class="font-semibold text-primary-700 mt-0.5">{{ getDepartmentName(form.department) }}</p>
                </div>
              </div>
            </div>
              ...
              <div v-for="b in bookingHistory" :key="b.id" class="py-4 first:pt-0 last:pb-0 flex items-center justify-between text-base">
                <div>
                  <div class="flex items-center gap-2">
                    <span class="font-bold text-gray-800">Bác sĩ: {{ b.doctorName }}</span>
                    <span class="text-xs bg-primary-100 text-primary-800 px-2 py-0.5 rounded font-bold">LH-{{ b.id }}</span>
                  </div>
                  <p class="text-sm text-gray-500 mt-0.5">Giờ khám: <strong class="text-gray-700">{{ b.khungGio }}</strong> - Ngày: {{ b.ngayHen }}</p>
                  <p class="text-xs text-gray-400">
                    STT: <span class="font-bold text-primary-700 mr-2">{{ b.id % 30 + 1 }}</span> 
                    Trạng thái: <span class="font-semibold" :class="b.trangThai === 'DA_XAC_NHAN' ? 'text-emerald-600' : b.trangThai === 'DA_HUY' ? 'text-red-500' : 'text-amber-600'">
                      {{ mapStatusText(b.trangThai) }}
                    </span>
                  </p>
                </div>

                <button
                  v-if="b.trangThai !== 'DA_HUY'"
    ...
    <div v-if="showConfirmModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
      <div class="bg-white rounded-3xl shadow-2xl max-w-md w-full p-6 md:p-8 animate-fade-in-up space-y-6">
        ...
        <div class="bg-gray-50 p-5 rounded-2xl border border-gray-100 text-base space-y-3">
          ...
          <p class="flex justify-between border-b pb-1.5"><span class="text-gray-500">Khoa khám:</span> <strong class="text-primary-800 font-bold">{{ getDepartmentName(form.department) }}</strong></p>
    ...
    <div v-if="showPaymentQRModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
      <div class="bg-white rounded-3xl shadow-2xl max-w-md w-full p-6 md:p-8 animate-fade-in-up space-y-6 border border-gray-100 text-center">
        ...
        <div class="bg-primary-50 p-4 rounded-2xl text-left text-sm text-primary-850 space-y-1">
          <p><strong>Nội dung:</strong> Đóng tiền khám bệnh - {{ tempBookingData?.patientName }}</p>
          <p><strong>Khoa khám:</strong> Khoa {{ getDepartmentName(form.department) }}</p>
