...
function loadPrescriptions() {
  // Mock some default prescriptions if none exists yet, so that the patient can immediately see something
  const mockPrescriptions = [
    {
      ...
      status: 'Chờ gọi tên', // Chờ gọi tên, Đã nhận thuốc
      medicines: [
        ...
        { name: 'Pharmaton Essential', qty: 15, unit: 'Viên', use: 'Uống trưa 1 viên sau ăn' }
      ]
    },
    {
      id: 2,
      ...
      medicines: [
        { name: 'Nexium mups 20mg', qty: 14, unit: 'Viên', use: 'Uống sáng 1 viên trước ăn 30 phút' },
        { name: 'Phosphalugel (Thuốc chữ P)', qty: 20, unit: 'Gói', use: 'Uống 1 gói khi đau hoặc sau ăn 2 tiếng' }
      ]
    }
  ]
  ...
  if (data) {
    prescriptions.value = JSON.parse(data)
  } else {
    prescriptions.value = mockPrescriptions
    localStorage.setItem('patientPrescriptions', JSON.stringify(mockPrescriptions))
  }
}
